...
def traceroute(host: str, max_hops: int = 30, pings_per_hop: int = 1, verbose: bool = False) -> str:
    ...
    results = [f"{'Hop':>3} {'Address':<15} {'Min (ms)':>8}   {'Avg (ms)':>8}   {
        'Max (ms)':>8}   {'Count':>5}"]
    ...
    for ttl in range(1, max_hops + 1):
        ...
        if ping_times:
            ...
            results.append(f"{ttl:>3} {addr[0] if addr else '*':<15} {min_time:>8.2f}ms {
                           avg_time:>8.2f}ms {max_time:>8.2f}ms {count:>5}")
        else:
...
print(f"Google DNS (ping): {ping_addr[0]} - {ping_time:.2f} ms" if (
    ping_addr and ping_time is not None) else "Google DNS (ping): Request timed out or no reply received")
...
print(f"HTTP URL: {http_url}, HTTP server status: {http_server_status}, Status Code: {
      http_server_response_code if http_server_response_code is not None else 'N/A'}")

https_url = "https://example.com"
https_server_status, https_server_response_code, description = check_server_https(
    https_url)
print(f"HTTPS URL: {https_url}, HTTPS server status: {https_server_status}, Status Code: {
      https_server_response_code if https_server_response_code is not None else 'N/A'}, Description: {description}")

# NTP Usage Example
print("\nNTP Example:")
ntp_server = 'pool.ntp.org'  # Replace with your NTP server
ntp_server_status, ntp_server_time = check_ntp_server(ntp_server)
print(f"{ntp_server} is up. Time: {
      ntp_server_time}" if ntp_server_status else f"{ntp_server} is down.")
...
for dns_query, dns_record_type in dns_queries:
    ...
    print(f"DNS Server: {dns_server}, Status: {dns_server_status}, {
          dns_record_type} Records Results: {dns_query_results}")
...
print(f"Server: {tcp_port_server}, TCP Port: {tcp_port_number}, TCP Port Status: {
      tcp_port_status}, Description: {tcp_port_description}")

print("\nTCP Port Example:")
tcp_port_server = "127.0.0.1"
tcp_port_number = 12346
tcp_port_status, tcp_port_description = check_tcp_port(
    tcp_port_server, tcp_port_number)
print(f"Server: {tcp_port_server}, TCP Port: {tcp_port_number}, TCP Port Status: {
      tcp_port_status}, Description: {tcp_port_description}")
...
print(f"Server: {udp_port_server}, UDP Port: {udp_port_number}, UDP Port Status: {
      udp_port_status}, Description: {udp_port_description}")

# UDP Port Usage Example
print("\nUDP Port Example:")
udp_port_server = "127.0.0.1"
